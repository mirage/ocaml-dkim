...
struct
  ...
  let server stack ?cache_size ?nameservers ?timeout dkim =
    let dns = DNS.create ?cache_size ?nameservers ?timeout stack in
    Lwt_scheduler.prj
  ...
  let verify ?newline ?cache_size ?nameservers ?timeout stream stack =
    let dns = DNS.create ?cache_size ?nameservers ?timeout stack in
  ...
  let verify ?newline ?cache_size ?nameservers ?timeout stream stack =
    Lwt_scheduler.prj (verify ?newline ?cache_size ?nameservers ?timeout stream stack)
end

...
let run _ src dst newline private_key seed selector hash canon domain_name =
  ...
  | _ ->
    let key = match private_key, seed with
      | Some (`RSA pk), _ -> pk
      | None, Some (`Seed seed) -> priv_of_seed seed
      | _ -> assert false (* see below *) in
    let flow = Flow.of_input src in
    let dkim = Dkim.v ~selector ?hash ?canonicalization:canon domain_name in
    let dkim = Unix_scheduler.prj (Dkim.sign ~key ~newline flow unix (module Flow) dkim) in
    match dst with
    | `Output ->
      let new_line = match newline with
        | Dkim.LF -> "\n" | Dkim.CRLF -> "\r\n" in
    ...
    | `Path path ->
      ...
      let new_line = match newline with
        | Dkim.LF -> "\n" | Dkim.CRLF -> "\r\n" in
      output_string oc (Prettym.to_string ~new_line Dkim.Encoder.as_field dkim) ;
      output_string oc (Buffer.contents flow.buffer) ;
      close_out oc ; Flow.close flow ; `Ok ()
...
let output =
  let parser str = let open Rresult in Fpath.of_string str >>| fun v -> `Path v in
  let pp ppf = function
...
let private_key =
  let parser str =
    let open Rresult in
    match Base64.decode ~pad:true str >>| Cstruct.of_string >>= X509.Private_key.decode_der with
    | Ok _ as v -> v
    | Error _ -> match Fpath.of_string str with
...
let hash =
  let parser str = match Astring.String.trim (String.lowercase_ascii str) with
    | "sha1" -> Ok `SHA1
    | "sha256" -> Ok `SHA256
...
let canon =
  let parser str =
    ...
    | _ -> Rresult.R.error_msgf "Invalid canonicalization specification: %S" str in
  let pp ppf = function
...
let seed =
  let parser str = match Base64.decode ~pad:true str with
    | Ok v -> Ok (`Seed v)
    | Error _ as err -> err in
...
let src =
  let doc = "The email to sign, if it's omitted, we expect something into the \
             standard input." in
  Arg.(value & pos ~rev:true 0 input `Input & info [] ~docv:"<input>" ~doc)

let dst =
  let doc = "The output file where we will store the signed email. If it's omitted, \
             we write on the standard output." in
...
let seed =
  let doc = "Seed to generate a private key. Instead to pass a private-key, the user can give \
             a seed used then by a Fortuna random number generator to generate a RSA private-key. \
             From the seed, the user is able to reproduce the same RSA private-key \
             (and the public-key). " in
  Arg.(value & opt (some seed) None & info [ "seed" ] ~doc)

let selector =
  let doc = "DKIM selector. A domain (see $(b,domain)) can store several public-key. \
             Each of them are identified by a $(i,selector) such as the public-key is stored \
             into $(i,selector)._domainkey.$(i,domain). It can refer to a date, a location \
             or an user." in
  Arg.(required & opt (some domain_name) None & info [ "s"; "selector" ] ~doc)

let hash =
  let doc = "Hash algorithm to digest header's fields and body. User can digest \
             with SHA1 or SHA256 algorithm." in
  Arg.(value & opt (some hash) None & info [ "hash" ] ~doc)

let canon =
  let doc = "Canonicalization algorithm used to digest header's fields and body. \
             Default value is $(i,relaxed/relaxed). A $(i,simple) canonicalization can be
             used. The format of the argument is: $(i,canon)/$(i,canon) or $(i,canon) to
             use the same canonicalization for both header's fields and body." in
  Arg.(value & opt (some canon) None & info [ "c" ] ~doc)

let hostname =
  let doc = "The domain where the DNS TXT record is available (which contains the public-key)." in
...
let sign =
  let doc = "Sign an email with a private-key and re-export the given email with a DKIM-Signature." in
  let exits = Term.default_exits in
  let man =
    [ `S Manpage.s_description
    ; `P "$(b,sign) permits to sign with a private-key (RSA) an email and re-export it with a proper \
          DKIM-Signature. The $(i,hostname) specified with its $(i,selector) must be reachable \
          by a DNS client and it must contain a TXT record with, at least, the public-key."
    ; `P "The output can used as is to a command which is able to send an email (such as $(b,sendmail))." ] in
  Term.(ret (const run $ setup_logs $ src $ dst $newline $ private_key $ seed $ selector $ hash $ canon $ hostname)),

...
module LwtIO = Dkim.Sigs.Make (struct
  type +'a t = 'a Lwt.t
end)
...
let lwt =
  { Dkim.Sigs.bind = lwt_bind; return = (fun x -> LwtIO.inj (Lwt.return x)) }
...
let main () =
  ...
  | Ok extracted ->
      ...
      Dkim.extract_body Lwt_io.stdin lwt
        (module Lwt_flow)
      ...
      Lwt_list.map_p
        (LwtIO.prj <.> Dkim.extract_server dns lwt (module Dns))
        mvalues
      >>= fun svalues ->
      let svalues =
        List.map

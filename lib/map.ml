...
module K = struct
  ...
  let a : (algorithm * hash) key = Key.create
  ...
  let h : field_name list key = Key.create { name= "field"; pp= Fmt.Dump.list Value.pp_field }
  ...
  let s : selector key = Key.create { name= "selector"; pp= Value.pp_selector }
  let t : int64 key = Key.create { name= "timestamp"; pp= Fmt.int64 }
  let x : int64 key = Key.create { name= "expiration"; pp= Fmt.int64 }
  let z : copies key = Key.create { name= "copies"; pp= Fmt.Dump.list Value.pp_copy }
  let sv : server_version key = Key.create { name= "server-version"; pp= Fmt.string }
  let sh : hash list key = Key.create { name= "hashes"; pp= Fmt.Dump.list Value.pp_hash }
  let k : algorithm key = Key.create { name= "algorithm"; pp= Value.pp_algorithm }
  let p : base64 key = Key.create { name= "public-key"; pp= Fmt.string }
  let n : string key = Key.create { name= "notes"; pp= Fmt.string }
  let ss : service list key = Key.create { name= "services"; pp= Fmt.Dump.list Value.pp_service }
  let st : name list key = Key.create { name= "names"; pp= Fmt.Dump.list Value.pp_name }
end
